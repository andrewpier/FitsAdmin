...
class VotingViewController: UIViewController {
   // var numberOfCards: UInt = 10
    ...
    var mostRecentPost:Int {
        get {
            if let _  = votingObject {
                return Int(votingObject!["AdminNumber"].intValue)
            } else {
                return 0
            }
        }
        ...
        set (value) {
            if let _ = votingObject {
                votingObject!["AdminNumber"] = value
    ...
    var tutorialOverlay: VotingTutorialView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //TO DO: Disable tab bar so only tutorial can be interacted with
...
extension VotingViewController: KolodaViewDataSource {
        ...
        query.findObjectsInBackgroundWithBlock {
            ...
            if error == nil {
                ...
                if let _ = objects {
                    ...
                    if (objects!.count < Int(numberOfCardsForThisFile)){
                        for index in 0..<(Int(numberOfCardsForThisFile) - objects!.count) {
    ...
    func getMostRecentPostIDNumber(){
        ...
        query.whereKeyExists("AdminNumber")
        query.findObjectsInBackgroundWithBlock {
            (objects: [PFObject]?, error: NSError?) -> Void in
            if error == nil {
                // The find succeeded.
    ...
    func createVotingItem(){
        ...
        gameScore["AdminNumber"] = self.mostRecentPost
        do {
            try gameScore.pin()
        } catch _ {
